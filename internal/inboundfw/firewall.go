...
//
// Note 是**同步**调的，握手洪泛下会与 Accept 里的 isBanned 在 f.mu 上串行。这一点是知情的、
// 刻意保留的：临界区里只有几次 map 操作（配置快照与日志都已经挪到锁外，见 Note 的说明），
// 且每条失败连接只来一次——代价随"连接出错的速率"增长，而不是随请求量增长。
//
// 不改成异步（投队列 + 后台 goroutine 消费）的理由：队列要么无界（洪泛时就是内存增长，
// 把一个连接层的问题换成一个进程级的问题），要么有界而在满时丢弃。而"满"发生的时刻
// 恰好就是洪泛正在进行的时刻——丢掉的是自动封禁最需要的那些计数，攻击者反而因为
// 打得够快而数不满阈值。宁可让这条链路串行一会儿。
