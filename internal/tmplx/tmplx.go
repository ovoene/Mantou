...
// 进执行日志、进 HTTP 请求体。
//
// 64 KB 这个数是这样定的：它要**明显高于**各家群机器人自己的上限
// （钉钉 markdown 约 20000 字节，企业微信 text 2048 字节，Telegram 4096 字符），
// 这样这道闸就只在"失控"时才响，而不会去截一条本来就发得出去的长消息——
// 真正把消息截短的应该是收件方那条明确的限制，用户看得懂它，而不是本程序的内部数字。
// 上界则由内存决定：单条渲染结果乘以并发上限（webhook 的 maxInflight = 64）
// 才是真正会同时驻留的量，64 KB × 64 ≈ 4 MB 是可接受的最坏值，再往上加就不是了。
//
// 这个常量也顺带定了模板草稿本身的长度上限（见 server.previewTemplateLimit）。
