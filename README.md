...
其余依赖都跟着上游走（vite 8、pinia 4、vue-router 5、vue-i18n 11、echarts 6、gopsutil v4 都是当时最新），只有下面三项是**决定**，不是忘了升。`package.json` 与 `go.mod` 里看不出这个区别，所以写在这里；顺带把「什么时候该回来重看」一并写下——不写触发条件的话，一个当时成立的判断会靠惯性变成永久冻结。

| 停在 | 为什么 | 什么时候回来重看 |
| --- | --- | --- |
| `typescript` 5.9.x | TypeScript 7 是 Go 重写的 tsgo，npm 包不再导出 `./lib/tsc`，而 `vue-tsc` 内嵌 TS 的 compiler API，在它上面直接崩。`vue-tsc` 的 peer 范围写着 `typescript >=5.0.0`，那个宽松是假的。 | `vue-tsc` 的 release note 明确写支持 tsgo / TS 7 之后。判据是现成的：`cd web && npm i -D typescript@7 && npx vue-tsc --noEmit` 能过就能升，且必须与 `vue-tsc` 同一次升。 |
| `vue` 3.5.x | 3.6 换了响应式实现，而它带来的那些能力（Vapor 之类）这个面板一处都用不上——收益抵不过整套界面重跑一遍回归的成本。 | Vue 3.5 停止安全修复，或 element-plus / vue-router / pinia 中任何一个把 peer 提到 `>=3.6`（`npm ls vue` 会直接报 peer 冲突）。 |
| `gin` v1.10.1 | 同样是收益不抵回归面：面板的每个路由、中间件、multipart 上传都要重验一遍。 | CI 里那个「依赖漏洞扫描（不阻断）」作业的 `govulncheck ./...` 开始报到 gin，或本项目要用只有新版才有的 API。安全公告一出就升，那时不必再权衡。 |

还有一处刻意的不一致，别顺手拉平：**工具链下限**与**实际构建版本**是分开的——`go.mod` 写 `go 1.26.0` 而 CI 与镜像用 Go 1.27，`package.json` 的 `engines.node` 写 `>=22.12.0` 而 CI 与镜像用 Node 24。低的那个是"用户至少要有什么才能自己编译"，高的那个是"我们用什么编译"，两者拉平会无谓地抬高源码构建的门槛。
...
| 依赖 | 版本要求 | 用途 |
|------|---------|------|
| Go | ≥ 1.26（go.mod 下限，CI/镜像用 1.27） | 编译后端二进制 |
| Node.js + npm | ≥ 22.12（package.json engines 下限，CI/镜像用 24 LTS） | 构建前端 `web/dist` |
...
多阶段构建：`node:24-alpine`（前端）→ `golang:1.27-alpine`（后端）→ `alpine:3.24`（运行），最终镜像约 18–25 MB。国内网络卡在拉基础镜像时，用 `MIRROR` 前缀切换加速站。

三个基础镜像都在 Dockerfile 里按 `@sha256:` digest 钉死，所以 `MIRROR` 只改变**从哪里取**，不改变**取到什么**（加速站给错内容会直接构建失败；Go 依赖由 `go.sum` 校验，npm 依赖由 `package-lock.json` 的 integrity 校验）。但自行构建的镜像不是官方产物——官方镜像只由 GitHub Actions 构建并推送到 `ghcr.io`，附带构建来源证明；自建镜像会把用过的镜像源写进 `io.mantou.build.*` 标签，`docker image inspect` 可查。

Release 附件同样带构建来源证明，可校验它确实出自本仓库的发布流水线：
...
1. **用面板导出（推荐，换机就能用）**：「设置 → 备份与恢复 → 导出配置」。导出的整个文件是加密的，密钥由**你的登录账户名 + 备份口令**推算出来（备份口令默认就是登录密码）。所以**换到新机器导进去就能直接用**：外观、皮肤、各模块规则、账号密钥一起回来，不需要把 `master.key` 一起带走，导入方会用它自己新生成的密钥重新加密存盘。代价是**忘了当时导出用的账号和口令就恢复不了**。
   > 导出弹窗里那栏「独立备份口令」是可选的：填了它，这份文件就只能用它解开，登录密码不再有效。要把备份交给别人保管、或者不想让"改了登录密码"和"手里那些旧备份"这两件事再对不上时用它——备份里存的是**明文凭证**（DDNS、Webhook、证书私钥全套），所以它至少要 8 个字节，越长越好。留空则一切照旧。
